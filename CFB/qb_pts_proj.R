...
qb_rushing_stats <- qb_rushing_stats[,-c(1,3,4,6,8,9,11,15,21:26,29:31,33)]
...
xgboost_qb_stats_model <- train(dk_pts ~ ., data = TrainingSet,
                             method = "xgbTree",
                             na.action = na.omit,
...
xgboost_qb_stats_model.training <-predict(xgboost_qb_stats_model, TrainingSet) 
xgboost_qb_stats_model.testing <-predict(xgboost_qb_stats_model, TestingSet) 


plot(TrainingSet$dk_pts,xgboost_qb_stats_model.training, col = "blue" )
plot(TestingSet$dk_pts,xgboost_qb_stats_model.testing, col = "blue" )

summary(xgboost_qb_stats_model)

xgboost_qb_stats_r.training <- cor(TrainingSet$dk_pts,xgboost_qb_stats_model.training)
xgboost_qb_stats_r.testing <- cor(TestingSet$dk_pts,xgboost_qb_stats_model.testing)

xgboost_qb_stats_r2.training <- xgboost_qb_stats_r.training^2
xgboost_qb_stats_r2.testing <- xgboost_qb_stats_r.testing^2

xgboost_qb_stats_actuals_preds <- data.frame(cbind(actuals=(TestingSet$dk_pts), predicteds=(xgboost_qb_stats_model.testing)))
xgboost_qb_stats_actuals_preds$diff <- (xgboost_qb_stats_actuals_preds$actuals - xgboost_qb_stats_actuals_preds$predicteds)

TestingSet <- cbind(TestingSet,xgboost_qb_stats_actuals_preds)

plot(TestingSet$actuals,TestingSet$predicteds, col = "blue" )

xgboost_qb_stats_min_max_accuracy <- mean(apply(xgboost_qb_stats_actuals_preds, 1, min) / apply(xgboost_qb_stats_actuals_preds, 1, max))  
xgboost_qb_stats_mape <- MAPE(xgboost_qb_stats_actuals_preds$predicteds, xgboost_qb_stats_actuals_preds$actuals)
xgboost_qb_stats_RMSE <- sqrt(mean((TestingSet$actuals - TestingSet$predicteds)^2))
xgboost_qb_stats_MAE <- mean(abs(TestingSet$actuals - TestingSet$predicteds))

TestingSet <- left_join(TestingSet, qb_stats)
saveRDS(xgboost_qb_stats_model, "xgboost_qb_stats_model.rds")

comp <- left_join(TestingSet,qb_stats)

sd(xgboost_qb_stats_model.testing)
